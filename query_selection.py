...
def pattern(inicial, data, nodes, metric, perc_act, perc_path,i, col_id):
        ...
        if (nodes == 'concept:name'):
            ...
            if metric in ['Mean Cycle Time', 'Median Cycle Time', 'StDev Cycle Time', 'Total Cycle Time']: 
                pattern = st.selectbox(
                        ...
                        'Identify transitions as bottlenecks',
                        'Identify transitions with high duration'), key=col_id)
            else:
                pattern = st.selectbox(
                        'Pattern search',
                        ('Identify DFGs by the number of unique resources', 
                        'Identify resources with high workload'), key=col_id)
            ...
            if metric in ['Mean Cycle Time', 'Median Cycle Time', 'StDev Cycle Time', 'Total Cycle Time']:  
                pattern = st.selectbox(
                    ...
                    'Identify activities as bottlenecks',), key=col_id)   
            else:
                pattern = st.selectbox(
                    'Pattern search',
                    ('Identify DFGs by the number of unique nodes',
    ...
    # return pattern
        ...
        if(pattern == 'Identify DFGs by the number of unique nodes' or pattern == 'Identify DFGs by the number of unique resources'):
            ...
            ['Minimum number of nodes', "Maximum number of nodes", 'Specific number of nodes'], key=f"param_{col_id}")
        ...
        #     param = st.number_input('Minimum minutes to consider an activity with high duration', step=1) 
            ...
            ['Mean frequency', "Less than 10 (frequency)", 'Other'], key=f"param_{col_id}")
            if param == 'Other':
            ...
            ['Mean frequency', "More than 10 (frequency)", 'Other'], key=f"param_{col_id}")
            if param == 'Other':
            ...
            ['Mean cycle time of transitions',  'Other'], key=f"param_{col_id}")
            if param == 'Other':
            ...
            ['Transition with the maximum duration',  'Other'], key=f"param_{col_id}")
            if param == 'Other':
            ...
            ['Mean frequency', 'Other'], key=f"param_{col_id}")
            if param == 'Other':
            ...
            ['Activity with the maximum duration',  'Other'], key=f"param_{col_id}")
            if param == 'Other':
            ...
            ['Maximum Cycle Time of resources',  'Other'], key=f"param_{col_id}")
            if param == 'Other':
...
def numberOfNodes(dic, param, num):
    ...
    for key, datos in dic.items():
        ...
        if param == 'argmin':
            ...
            if(len(graph.nodes) >= num):
                prueba[key] = datos
                prueba2[key] = len(graph.nodes)

    if param == 'argmin':
        st.markdown(f" **(Minimum number of nodes: {min_nodos})**")
        return prueba, prueba2
    elif param == 'argmax':
        st.markdown(f" **(Maximum number of nodes: {max_nodos})**")
        return prueba, prueba2
    else:
        param == 'Specific number of nodes'
        return prueba, prueba2
...
def function_numberOfNodes(dic, param, num):

    prueba = {}
    prueba2 = {}

    # 1️⃣ Calcular nodos por grafo
    nodos_por_grafo = {
        key: len(datos['graph'].nodes)
        for key, datos in dic.items()
    }

    modo, valor = parse_num_expression(num)

    # 2️⃣ Casos argmax / argmin
    if param == 'argmax':
        # ordenar de mayor a menor
        seleccionados = sorted(
            nodos_por_grafo.items(),
            key=lambda x: x[1],
            reverse=True
        )[:valor]

        st.markdown(f" **(Top {num} maximum number of nodes)**")
        ...
        st.markdown(f" **(Top {num} minimum number of nodes)**")

    elif modo == 'gt':
        seleccionados = [
            (k, n) for k, n in nodos_por_grafo.items()
            if n > valor
        ]

        st.markdown(f"**Number of nodes > {valor}**")
...
def parse_num_expression(expr):
    ...
    # asignación: k=3 o n=2
    m = re.match(r'\w+\s*=\s*(\d+)', expr)
    if m:
        return 'assign', int(m.group(1))
    ...
    raise ValueError(f"Expresión numérica no válida: {expr}")
    


def pattern_arguments(inicial, nodes, metric, perc_act, perc_path,i):

    selected = {}
    selected_data = {}

    data = st.session_state.collections
    ...
    if data is not None:
        ...
        if (nodes == 'concept:name'):
            if metric in ['Mean Cycle Time', 'Median Cycle Time', 'StDev Cycle Time', 'Total Cycle Time']:  
                pattern = st.selectbox(
                        ...
                        'Identify activities with high duration',   # impememtado pero falta comprobar que existe tiempo asociado a actividades
                        'Identify transitions as bottlenecks',
                        'Identify activities as bottlenecks'), key=i)    # impememtado pero falta comprobar que existe tiempo asociado a actividades
            else:
                pattern = st.selectbox(
                        'Pattern search',
                        ('Difference in number of nodes', 'Otro'), key=i)
        ...
        #                 'Identify resources as bottlenecks',
        #                 'Identify transitions as bottlenecks',
        #                 'Identify transitions with high duration'), key=i)
        #     else:
        #         pattern = st.selectbox(
        #                 'Pattern search',
        #                 ('Identify DFGs by the number of unique resources', 
        #                 'Identify resources with high workload'), key=i)
        ...
        #             'Identify transitions with high duration',
        #             'Identify activities with high duration',
        #             'Identify activities as bottlenecks',), key=i)   
        #     else:
        #         pattern = st.selectbox(
        #             'Pattern search',
        #             ('Identify DFGs by the number of unique nodes',
        #             'Identify infrequent activities',
        #             'Identify the most frequent activities'), key=i)

        param = st.selectbox('Optimization',  ['Minimum', "Maximum"], key=f"param_{i}")
        num = st.number_input('Number:', step=1, min_value=0)
    ...
    selected_collections = {
        cid: data[cid] for cid in ids if cid in data}
    
    func = PATTERN_FUNCTIONS.get(pattern)
    
    for id, coll in selected_collections.items():
        res[id] = {"description": coll["description"], "results": {} }
    ...
    for i, col in selected_collections.items():
        for key, datos in col["dfgs"].items():
...
def dif_numberNodes(dfg):
    """
    Returns the number of nodes in a DFG.
    """
    return len(dfg['graph'].nodes)


# def compare_collections(res, ref_cid, target_cid, mode, top_k):
#     """
#     Compare two collections by absolute difference of metric values.

#     mode: "min" or "max"
#     """
#     ref_results = res[ref_cid]["results"]
#     target_results = res[target_cid]["results"]

#     ref_value = next(iter(ref_results.values()))

#     diffs = []

#     for dfg_id, value in target_results.items():
...
#     reverse = mode == "Maximum"
#     diffs_sorted = sorted(diffs, key=lambda x: x["difference"], reverse=reverse)

#     st.write()

#     return diffs_sorted[:top_k]
...
def compare_collections(res, ref_cid, target_cid, mode, top_k):
    ...
    ref_dfg_id, ref_value = next(iter(ref_results.items()))

    diffs = []

    for dfg_id, value in target_results.items():
        if dfg_id == ref_dfg_id:
            continue
...
PATTERN_FUNCTIONS = {
    "Difference in number of nodes": dif_numberNodes
    # añade aquí las que quieras
}       
